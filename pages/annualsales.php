            ...
            <script>
              ...
              <div class="col-lg-12 grid-margin">
                <div class="card">
                    ...
                    <div class="card-body">
                         <h4 class="card-title">Products Sold This Year</h4>
                        <div class="card-heading"></div>
                          ...
                          <table class="table table-bordered table-striped" id="first">
                                    ...
                                    <tbody>
                                        <?php
										  $da = date("d/m/Y");
                                               $resultt = mysqli_query($con, "SELECT product, price, sum(qnty) as qnt,sum(total) as total FROM cart WHERE YEAR(dated) = YEAR(CURDATE()) GROUP BY product ORDER BY dated DESC "); 
                                         ...
                                         <?php
										  $da = date("d/m/Y");
										       $productsqquery = "SELECT 
                                                    ...
                                                    YEAR(STR_TO_DATE(thisday, '%d/%m/%Y')) AS thisyear,
                                                    SUM(bp) AS cost,
                                                    bp,
                                                    sp
                                                FROM profits
                                                WHERE YEAR(STR_TO_DATE(thisday, '%d/%m/%Y')) = YEAR(CURDATE())
                                                GROUP BY product
              ...
              <div class="mt-3">
                <div class="card">
                  <div class="card-body">
                    <h4 class="card-title">VAT This Year is Ksh. 
                         <?php
                             $da = date("Y-m");
                             $yuiit = mysqli_query($con,"select SUM(total) as total from cart where year(dated) = year(CURDATE()) and tax='1' GROUP BY product");
                             $taxs = 0;
                        ...
                        <thead>
                            <tr>
                                ...
                                <tbody>
                                         <?php
    										  $da = date("Y-m");
    										  $yuiit = mysqli_query($con,"select SUM(total) as total,product from cart where year(dated) = year(CURDATE()) and tax='1' GROUP BY product");
    										  while($rowg = mysqli_fetch_array($yuiit)){
    										      echo "<tr>";
    										      $tax = $rowg['total']*0.16;
    										      echo '<td>'.$rowg['product'].'</td>';
