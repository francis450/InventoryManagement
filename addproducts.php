    ...
    <div class="content-wrapper" style="background:#ffffff">
        <div class="">
            ...
            <table class="table display nowrap" style="width: 100%" id="example">
                            <thead>
                                <tr>
									<th style="color:red">Supplier</th>
									<th style="color:red">Invoice No.</th>
									<th style="color:red">Invoice Date</th>
									<th style="color:red">Cost</th>
									<th style="color:red">Amount Paid<br>
										<span>(To supplier)</span>
									</th>
									<th>Balance(To be paid)</th>
									<th class="must" style="color:red;display: flex;flex-direction: column;align-items: center;">
											Taxable(16%)<br>
										<span>Y/N</span>
									</th>
                                </tr>
                            ...
                            <tbody>
                                <tr>
									...
									<td style="    display: flex;align-items: baseline;">
										<select name="supplier" id="supplier">
											...
											<?php
													$suppliers = mysqli_query($con, "SELECT DISTINCT * FROM suppliers where supplier != '' order by supplier asc");
													
													while($allsuppliers = mysqli_fetch_array($suppliers)){
													    $suppp = $allsuppliers['supplier'];
														echo '<option value='.$suppp.'>'.$suppp.'</option>';
													}
											?>
										</select>
										<div class="addsupplier" style="cursor:pointer">+</div>
									</td>
									<!-- Invoice Number -->
									<td>
										<input type="text" name="invoice" id="innvoice">
									</td>
									<!-- Invoice Date -->
									<td>
									...
									<!--amount paid-->>
									<td>
										<input type="number" name="paidToSupplier" id="paidToSupplier">
									</td>
									<!--Balance-->
									<td>
										<input type="number" name="balanceToSupplier" id="balanceToSupplier" disabled>
									</td>
									<!-- TAxable -->
								...
								<tr>
									<thead>
										...
										<th>Stock</th>
										<th  style="color:red">Barcode</th>
										<th style="color:red">Serial No.<br></th>
										<th style="color:red; display: flex;justify-content: center;align-items: baseline">Category</th>                    
										<th  style="color:red">Subcategory</th>
										<th  style="color:red">Units<br>
                                        	<span>(eg.No. of boxes)</span> 
                                    	</th>
										<th style="color:red">Sub Units<br>
											<span>(eg. pieces per box)</span>
										</th> 
										<th>Total Pieces</th>
									</thead>
									<tbody>
										<tr>
											...
											<td style="display: flex;align-items: baseline;">
												<select name="product" id="product">
													<option>--select--</option>
													<?php
														$prod = mysqli_query($con, "SELECT * FROM inventory ORDER BY brand ASC");
														while($prods = mysqli_fetch_array($prod)){
															echo '<option value="'.$prods['brand'].'">'.$prods['brand'].'</option>';
														}
													?>
												</select>
												<div class= 'addproduct'style="cursor:pointer">+</div>
											</td>
											<!--Stock-->
											<td>
											    <input type='number' style="width: 80px" name='stock' id='stock' readonly>
											</td>
											<!-- Barcode -->
											<td>
												<input type="text" style="width: 150px" name="barcode" id="barcode">
											</td>
											<!--Serial No.-->
											<td>
												<input type="text" style = "width:90px" name="serialnumber" id="serialnumber">
											</td>
											<!-- Category -->
											<td style="display:flex;align-items:baseline">
												...
												<select name="category" id="category" class="category">
													...
													<?php
														$categories = mysqli_query($con, "SELECT distinct category FROM inventory ORDER BY category DESC");
														while($allcategories = mysqli_fetch_array($categories)){
														    $cattt = $allcategories['category'];
															echo '<option value='.$cattt.' >'.$cattt.'</option>';
														}
													?>
												</select>
												<div class="addcategory" style="cursor:pointer;">+</div>
											</td>
											<!-- Subcategory -->
											<td style="display:flex;align-items:baseline">
												<select name="subcategory" id="subcategory" class="subcategory">
													...
													<?php 
														$d = mysqli_query($con,"select distinct subcategory from inventory order by subcategory DESC");
                                                		while($row=mysqli_fetch_array($d)){
                                                			echo '<option>'.$row['subcategory'].'</option>';
                                                		}
													?>
												</select>
												<div class="addsubcategory" style="cursor:pointer;">+</div>
											</td>
											<!--Units-->
											<td>
												<input type="number" style="width: 90px" name="units" id="units">
											</td>
											<!--Sub Units-->
											<td>
												<input type="number"  style="width: 90px" name="subunits" id="subunits">
											</td>
											<!-- Total Pieces -->
											<td>
												<input type="number" style='width:90px' name="totalPieces" id="totalPieces" disabled>
											</td>
								...
								<tr>
									<thead>
										<th>Cost/Unit<br>
											<span>(eg. Kshs per box)</span>
										</th>
										<th>Reorder Level</th>
										<th  style="color:red">Cost/Sub Unit<br>
											<span>(eg. Kshs per piece)</span>
										</th>
										<th  style="color:red">Measure</th>
										<th>Expiry Date</th>
										<th  style="color:red">Retail P.</th>
										<th >Wholesale P.</th>
										<th style="color:red; display: flex;justify-content: center;">
											Shelf
										</th>
									</thead>
									<tbody>
										<!--Cost/Unit-->
										<td>
											<input type="number" style="width:120px" name="costUnit" id="costUnit" disabled>
										</td>
										<!-- Reorder Level -->
										<td>
											<input style="width:100px" type="number" id="reorder">
										</td>
										<!--Cost/sub unit-->
										<td>
											<input style="width:120px" type="number" name="costSubUnit" id="costSubUnit" disabled>
										</td>
										<!-- measure -->
										<td style="display:flex;align-items:baseline">
												<option></option>
												<select name="measure" id="measure">
													<option value="measure">--select--</option>
													<?php
														$measures = mysqli_query($con, "select distinct measure from inventory order by measure DESC");
														while($mm = mysqli_fetch_array($measures)){
														    $mme = $mm['measure'];
															echo '<option value='.$mme.'>'.$mme.'</option>';
														}
													?>
												</select>
												<p id="addMeasure" style="cursor:pointer;">+</p>
										</td>
										<!--Expiry-->
										<td>
										...
										<!--Retail Price-->
										<td>
											<input type="number" style="width:130px" name="retail" id="retail">
										</td>
										<!--Wholesale price-->
										<td>
											<input type="number" style="width:130px" name="wholesale" id="wholesale">
										</td>
										<!--Shelf-->
										<td style="display:flex;align-items:baseline">
											<!--  -->
											<select name="shelf" id="shelf" style="display:flex;align-items:baseline">
												<option>--select--</option>
												<?php
													$ret = mysqli_query($con, "SELECT distinct shelf FROM inventory where shelf != '' ORDER BY shelf ASC");  // Corrected order by clause
													while($rets = mysqli_fetch_array($ret)){
														echo '<option>'.$rets['shelf'].'</option>';
													}
												?>
											</select>
											<div class="addshelf" style="cursor:pointer;">+</div>
										</td>
